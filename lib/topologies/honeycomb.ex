defmodule Honeycomb do
  ...
  def start_link(_no_of_nodes) do
    Agent.start_link(fn -> %{} end, name: :honeycomb)
  end

  def value do
    Agent.get(:honeycomb, & &1)
  end

  def delete_node(node_id) do
    neighbours_list = Honeycomb.value()
  ...
  def getNeighbours(node_id) do
    Honeycomb.value() |> Map.get(node_id)
  end
  ...
  def update_map(node_id, neighbours) do
    Agent.update(:honeycomb, &Map.put(&1, node_id, neighbours))
  end
